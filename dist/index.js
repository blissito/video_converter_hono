...
import { getPutFileUrl, getReadURL } from 'react-hook-multipart';
import fs from 'fs';
 ...
 */
const buildBitrateParameters = ({ versions, encodingSpeed, frameRate, }) => {
    const args = [];
    versions.forEach((q, i) => {
 ...
 */
function convertMP4({ onEnd, videoSourcePath, storageKey, versions, segmentSize = 6, // secs
...
const transcodeDetached = async ({ Bucket, storageKey, onError, onEnd, }) => {
    const agenda = new Agenda({
        db: { address: process.env.DATABASE_URL },
    });
