 ...
 */
function convertMP4({ onEnd, onError, onStart, videoSourcePath, storageKey, versions, segmentSize = 6, // secs
...
const handleDeleteAllChunks = async (c) => {
    ...
    agendar(async () => {
        ...
        console.info("::OBJECTS_DELETED::", resul.Deleted.length, "webhook::" + webhook);
        webhook &&
            (await callWebHook({
                webhook,
                eventName: "onDelete",
...
const setSegmentCounter = (number) => (segmentCounter = number);
